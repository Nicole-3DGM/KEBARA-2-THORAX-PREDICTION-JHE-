#Prediction Kebara 2 from N=60 H. sapiens: MODEL B
require(Morpho)
require(geomorph)
require(Rvcg)
require(ape)
...
Todo_trunk <- relaxarray[,,1:60]

# Thorax and pelvis n=60 H. sapiens
Thorax_temp <- read.morphologika("D:/R progress/Trunk_analyses_R/Kebara2_prediction/R1/Kebara_2_prediction/Thorax_temp_mlk_stern.txt")
Thorax_indices <- vcgKDtree(Trunk_data[,,26],Thorax_temp[,,1],k=1)$index
Thorax <- Todo_trunk[Thorax_indices,,]
Pelvis_temp <- read.morphologika("D:/R progress/Trunk_analyses_R/Kebara2_prediction/R1/Kebara_2_prediction/Pelvis_temp_mlk.txt")
Pelvis_indices <- vcgKDtree(Trunk_data[,,26],Pelvis_temp[,,1],k=1)$index
Pelvis <- Todo_trunk[Pelvis_indices,,]

Thorax_mesh <- vcgImport("D:/R progress/Trunk_analyses_R/Kebara2_prediction/R1/Kebara_2_prediction/Thorax_temp.ply", updateNormals = TRUE, readcolor = FALSE, clean = TRUE,silent = FALSE)
Pelvis_mesh <- vcgImport("D:/R progress/Trunk_analyses_R/Kebara2_prediction/R1/Kebara_2_prediction/Pelvis_temp.ply", updateNormals = TRUE, readcolor = FALSE, clean = TRUE,silent = FALSE)

# GPA DE 60 H. SAPIENS----

# Procrustes Superimposition (GPA) of the two blocks separately

Thorax_GPA_Homo <- procSym(Thorax[,,1:60])
Pelvis_GPA_Homo <- procSym(Pelvis[,,1:60])

#MODEL B: PLS N=60 Homo sapiens ----

PLS_ModelB <- pls2B(Thorax_GPA_Homo$rotated,Pelvis_GPA_Homo$rotated, rounds=999,cv=T)
PLS_ModelB

Trunk_sex <- name2factor(Trunk_data,which=3)
col_polyg <- c("black")
col_plot_sex <- c((c(rep("skyblue",4),rep("dodgerblue3",6),rep("skyblue",1),rep("dodgerblue3",2),rep("skyblue",2),rep("dodgerblue3",2),rep("skyblue",7),rep("dodgerblue3",12),rep("skyblue",18),rep("dodgerblue3",3),rep("skyblue",1),rep("dodgerblue3",2))))
col_leg_sex <- c("skyblue","dodgerblue3")

PLS_ModelB_comm <- getPLSCommonShape(PLS_ModelB)

plot(PLS_ModelB_comm$XscoresScaled[,1],PLS_ModelB_comm$YscoresScaled[,1],xlab="Block 1 (Thorax)",ylab="Block 2 (Pelvis)",main="TWO BLOCKS PLS ANALYSIS (1st latent variable)",pch=c(18,16)[as.numeric(Trunk_sex)],col=col_plot_sex,cex=2,xlim=c(-0.15,0.15),ylim=c(-0.2,0.2))
legend("bottomright", inset=.005, legend = c("Females","Males"), horiz=FALSE, cex=1.2,pt.cex=1.7,bty="n", col=col_leg_sex,pch=c(18,16))
col_polyg <- c("black")

groups60thorax <- read.table("D:/R progress/Trunk_analyses_R/Kebara2_prediction/R1/Kebara_2_prediction/Groups_N60sapiens.txt",header=TRUE)
if (require(car)) {
  for(ii in 1:length(levels(groups60thorax$SPECIES))){
    dataEllipse(PLS_ModelB_comm$XscoresScaled[groups60thorax$SPECIES==levels(groups60thorax$SPECIES)[ii],1],PLS_ModelB_comm$YscoresScaled[groups60thorax$SPECIES==levels(groups60thorax$SPECIES)[ii],1],add=TRUE,plot.points = F, center.pch = FALSE,levels=c(.95), col=col_polyg,fill=F, fill.alpha=0.10,robust=TRUE)}
}

abline(h=0.00,v=0.00,col="black")

Covar_ModelB <- plsCoVar(PLS_ModelB,1,1,1)

PLS1_ModelB_neg_x <- tps3d(Thorax_mesh,Thorax_temp[,,1],Covar_ModelB$x[,,1])
shade3d(PLS1_ModelB_neg_x, col="dodgerblue3",back="lines",box=FALSE,axes=FALSE,specular="white")

PLS1_ModelB_pos_x <- tps3d(Thorax_mesh,Thorax_temp[,,1],Covar_ModelB$x[,,2])
shade3d(PLS1_ModelB_pos_x, col="skyblue",back="lines",box=FALSE,axes=FALSE,specular="white")

PLS1_ModelB_neg_y <- tps3d(Pelvis_mesh,Pelvis_temp[,,1],Covar_ModelB$y[,,1])
shade3d(PLS1_ModelB_neg_y,col="dodgerblue3",back="lines",box=FALSE,axes=FALSE,specular="white")

PLS1_ModelB_pos_y <- tps3d(Pelvis_mesh,Pelvis_temp[,,1],Covar_ModelB$y[,,2])
shade3d(PLS1_ModelB_pos_y,col="skyblue",back="lines",box=FALSE,axes=FALSE,specular="white")
...
for (i in 1:dim(PLS_ModelB$predicted.x)[3]) # loop for cross-validation
  errorB <- c(errorB,mean((truth_thorax-PLS_ModelB$predicted.x[,,i])^2))

NoCVerror <- NULL
for (i in 1:dim(PLS_ModelB$predicted.x)[3]) # loop for MSE non-validated
  NoCVerror <- c(NoCVerror,mean((truth_thorax-vecx(predictPLSfromData(PLS_ModelB,y=Pelvis_GPA_Homo$rotated,ncomp=i)))^2))
...
points(1:58,NoCVerror,col="black",pch=15)                  
lines(1:58,NoCVerror,col="black")
legend(x="topright",y=NULL,legend=c("Cross-validated", "Non-validated"), col = c("red","black"),pch=c(19,15),lty=1)

#VALIDACIÓN DE LA PREDICCIÓN EN LA MUESTRA n=60 sapiens

cv <- 1:dim(Todo_trunk)[3]
predB <- Todo_trunk[Thorax_indices,,]*0
for (i in cv) {
  plsB <- pls2B(Thorax_GPA_Homo$rotated[,,-i],Pelvis_GPA_Homo$rotated[,,-i],rounds = 99)
  predB[,,i] <- predictPLSfromData(plsB,y=Pelvis_GPA_Homo$rotated[,,i],ncomp=2)
}

predB[,,1:60]

actual_pred_thorax <- bindArr(Thorax[,,1:60],pred_B[,,1:60],along=3)

r2morphologika(actual_pred_thorax,"actual_pred_thorax.txt")

#Best prediction Model B

Best_pred <- tps3d(Thorax_mesh,Thorax_temp[,,1],predB[,,57])
shade3d(Best_pred,col="azure",back="lines",box=FALSE,axes=FALSE,specular="white")
Orig_rot <- rotonto(Thorax_GPA_Homo$mshape,Thorax[,,57],scale = T)$yrot
...
#Worst prediction Model B

Worst_pred <- tps3d(Thorax_mesh,Thorax_temp[,,1],predB[,,23])
shade3d(Worst_pred,col="azure",back="lines",box=FALSE,axes=FALSE,specular="white")
Orig_rot <- rotonto(Thorax_GPA_Homo$mshape,Thorax[,,23],scale = T)$yrot
Orig <- tps3d(Thorax_mesh,Thorax_temp[,,1],Orig_rot)
...
Kebara2RA_toGPA_ModelB <- rotonto(Pelvis_GPA_Homo$mshape,Pelves_Kebara2[,,1],scale = T)$yrot
Pred_Kebara2RA_ModelB <- predictPLSfromData(PLS_ModelB,y=Kebara2RA_toGPA_ModelB,ncomp=2) #number of LVs to perform the prediction
deformGrid3d(Thorax_GPA_Homo$mshape,Pred_Kebara2RA_ModelB)
...
shade3d(Thorax_Kebara2RA_ModelB,col="azure",back="lines",box=FALSE,axes=FALSE,specular="white")
Thorax_Kebara2GO_mesh <- vcgImport("D:/R progress/Trunk_analyses_R/Kebara2_prediction/Kebara2_thorax_GO.ply", updateNormals = TRUE, readcolor = FALSE, clean = TRUE,silent = FALSE)
Thorax_Kebara2GO_toGPA_ModelB <- rotonto(Thorax_GPA_Homo$mshape,Kebara2_thorax_reconstructions[,,1],scale = T)$yrot
Thorax_Kebara2GO_ModelB <- tps3d(Thorax_Kebara2GO_mesh,Kebara2_thorax_reconstructions[,,1],Thorax_Kebara2GO_toGPA_ModelB)
...
shade3d(Thorax_Kebara2RA_ModelB,col="azure",back="lines",box=FALSE,axes=FALSE,specular="white")
Thorax_Kebara2S_M_mesh <- vcgImport("D:/R progress/Trunk_analyses_R/Kebara2_prediction/R1/Kebara_2_prediction/Kebara_thorax_SM.ply", updateNormals = TRUE, readcolor = FALSE, clean = TRUE,silent = FALSE)
Thorax_Kebara2S_M_toGPA_ModelB <- rotonto(Thorax_GPA_Homo$mshape,Kebara2_thorax_reconstructions[,,2],scale = T)$yrot
...
Kebara2SM_toGPA_ModelB <- rotonto(Pelvis_GPA_Homo$mshape,Pelves_Kebara2[,,2],scale = T)$yrot
Pred_Kebara2SM_ModelB <- predictPLSfromData(PLS_ModelB,y=Kebara2SM_toGPA_ModelB,ncomp=2) #number of LVs to perform the prediction
deformGrid3d(Thorax_GPA_Homo$mshape,Pred_Kebara2SM_ModelB)
...
shade3d(Thorax_Kebara2SM_ModelB,col="azure",back="lines",box=FALSE,axes=FALSE,specular="white")
Thorax_Kebara2S_M_mesh <- vcgImport("D:/R progress/Trunk_analyses_R/Kebara2_prediction/R1/Kebara_2_prediction/Kebara2_thorax_SM.ply", updateNormals = TRUE, readcolor = FALSE, clean = TRUE,silent = FALSE)
Thorax_Kebara2S_M_toGPA_ModelB <- rotonto(Thorax_GPA_Homo$mshape,Kebara2_thorax_reconstructions[,,2],scale = T)$yrot
Thorax_Kebara2S_M_ModelB <- tps3d(Thorax_Kebara2S_M_mesh,Kebara2_thorax_reconstructions[,,2],Thorax_Kebara2S_M_toGPA_ModelB)
shade3d(Thorax_Kebara2S_M_ModelB,col="palegreen",back="lines",box=FALSE,axes=FALSE,specular="white")
